...
- **У сервера выставлены таймауты.** `http.ListenAndServe` не ставит ни одного,
  и медленный клиент держит горутину бесконечно. Стоит четыре строки.
...
## Stress Test Results: план В1

### Resolved Decisions

- **Хранение геометрии остаётся цепочкой от якоря.** Решение владельца после
  разбора отраслевой практики. Абсолютные позы на каждый элемент выводимы из
  цепочки, поэтому переход к ним позже — материализация кэша, а не слом формата.
  Порядок: сейчас цепочка (карту пишет рука), редактор впишет позы, тайлы придут
  с ростом мира.
- **Обратный ход по элементу — обращение одного жёсткого движения**, а не
  разворот цепочки по звеньям. `geom` получает `Compose` и `Invert`.
- **Порт обслуживает до двух элементов.** Три и больше — ошибка про
  неоформленную стрелку.
- **Таймауты сервера ставятся сразу.**

### Changes Made

- Задача 1 расширена: `Compose`, `Invert` и тест «цепочка есть жёсткое движение».
- Задача 4: `used[end] > 2` вместо `> 1`; длина горизонтали считается одной
  функцией; ошибки `MetersToDistance` не глотаются; добавлена проверка формы
  геопривязки.
- Задача 5: `LengthS` возвращает ошибку вместо частичной суммы.
- Задача 6: `Propagate` переписан — `portState` с владельцем, `across` через
  композицию, `walkReverse` удалён; добавлен положительный тест на сходящийся
  цикл; фикстуры используют общий порт стыка.
- Задача 7: golden-вектор на правило округления.
- Задача 10: тест не переписывает `strings.Contains`; таймауты сервера.

### Deferred / Parking Lot

- `Cache-Control: public` оставлен как есть: аутентификации в В1 нет, карта не
  секрет, а `public` позволяет кэширование прокси в LAN. Пересмотреть, когда
  появятся сессии и контент, зависящий от игрока.
- `http.MaxBytesReader` на тела запросов — в В2, вместе с командами: в В1 тел нет.
- Абсолютные позы элементов и тайлы — с редактором и ростом мира.

### Confidence Assessment

- **Общая: высокая.** Задачи 1–8 покрыты тестами, каждая проверяема отдельно.
- **Остаточная забота:** Задача 11 (карта станции) даёт рецепт, а не литеральный
  JSON. Развернуть его механически можно, приёмочный тест ловит ошибку, но это
  единственное место плана, где шаг описывает *что*, а не показывает *как*.
- **Что стресс-тест поймал и чего не поймала бы реализация:** декоративную
  проверку замыкания. Она бы прошла все тесты и молча пропускала разъехавшиеся
  стыки до первой станции с настоящим съездом.
