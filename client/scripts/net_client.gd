## NetClient — HTTP-труба клиента.
##
## «Единственное место, где клиент говорит с сервером» — так здесь было
## записано, и с 2026-08-14 это неверно: рядом появилась вторая труба, сокет
## канала команд (net_channel.gd). Деление между ними не по вкусу, а по природе
## ресурса: крупное неизменяемое (сеть, рельеф, ассеты) идёт обычным GET с ETag и
## immutable — оно статично и кэшируется по хешу; состояние партии приходит
## каналом само, потому что спрашивать его по кругу нельзя, темп опроса стал бы
## темпом мира.
##
## Прежняя запись обещала сокет «в В2 вместе с мутациями». Он пришёл раньше и по
## другой причине: веха расщеплена (ClearAhead-wa51), канал отделён от стрелок —
## машинисту нужен канал, а не диспетчерская половина.
##
