...
## мира).
const ROLE_NOTES := {
	Role.DRIVER: "человек ростом 1.80 ходит по той же поверхности, которую видит · V — обзор, от первого лица, от третьего · поездов нет: подвижного состава нет в модели мира · на платформу с земли не взойти (лестниц нет), лес и трава проходимы насквозь, по реке ходят посуху",
