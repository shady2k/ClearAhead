...
static func frog_rail_mesh(rail: TrackBuild.FrogRail) -> ArrayMesh:
	...
	var bands := head_bands(rail.rail_head_width_m, rail.ride_full, rail.fillet_full)
...
static func frog_rail_fillet_mesh(rail: TrackBuild.FrogRail) -> ArrayMesh:
	if not rail.ready() or not _rides_on(rail.kind):
		return null
	return _ride_strip(rail, rail.fillet_full)
...
## ШИРИНА НАКАТА СНЕСЕНА ОТСЮДА 2026-08-18, и запись оставлена, чтобы её не
## завели заново.
##
## Здесь стояла RAILHEAD_WIDTH = 0.72 — доля головки, которую занимает чистая
## полоса. Довод под ней был не пустой: спайк выдумывал накат целиком (0.062 м
## рядом со своей же выдуманной головкой 0.071), и доля хотя бы ехала вместе с
## присланной головкой, а не жила своим числом про размер рельса.
##
## Отменил его не он сам, а ПОТРЕБИТЕЛЬ. Доля отвечает на вопрос «какая часть
## головки блестит», а вопрос стоит другой: ГДЕ ИДЁТ КОЛЕСО. Это решение о
## ходовой функции детали, и по закону эпика ClearAhead-ax7m показ его не
## принимает. Пока сервер молчал, доля была меньшим злом; с 2026-08-18 он
## объявляет полосу метрами (`ride_width` в корне ответа) — и вторая половина
## ответа стала лишней.
##
## ЦЕНА ПЕРЕЕЗДА В КАДРЕ РАВНА НУЛЮ НА ПЕРЕГОНЕ: 0.72 × 0.075 = 0.054 м — ровно
## то, что сервер теперь и объявляет. Изменился остряк и сердечник, где стояло
## 0.035: их полоса стала той же, что у нитки, с которой они стыкуются.

## ФАСКА СНЕСЕНА ОТСЮДА 2026-08-18, следом за накатом и по тому же слову
## владельца: «у клиента вообще такой информации быть не должно, только на
## сервере».
##
## Здесь стояла RAILHEAD_FILLET = 0.075 — доля ширины головки на каждую сторону.
## Сама фаска осталась и остаётся правильной: у настоящего рельса край головки
## стёрт, но не отполирован, и полоса там серая. Уехало ЧИСЛО: ширина фаски есть
## размер детали, а не решение показа, и приезжает она метрами в `fillet_width`
## (разбор — server/internal/track/ride.go).
##
## ДОЛЯ БЫЛА ХУЖЕ ЧЕМ ПРОСТО НЕ НА МЕСТЕ. Она не единица измерения, а ссылка на
## чужое число: 0.075 доли головки означает «зависит от того, что пришлют», — и
## приедь рельс другого профиля, фаска поехала бы следом без основания.
## Скругление края головки задаётся профилем проката, а не пропорцией.
##
## ЧТО ОСТАЛОСЬ ЗДЕСЬ ПО ПРАВУ: третий МАТЕРИАЛ. Переход выражен полосой, а не
## градиентом, потому что у путевых мешей нет ни развёртки, ни вершинных цветов,
## а металличность по вершинам не интерполируется вовсе — «полуметалла не
## бывает». Это решение о том, ЧЕМ рисовать, и оно показа.
...
##
## ОБА РАЗМЕРА ПРИХОДЯТ ЧИСЛАМИ, а не выводятся долями головки: и где идёт
## колесо, и где его след кончается — решения о ходовой функции, и принимает их
## сервер (`ride_width` и `fillet_width` в корне ответа, разбор — ride.go).
## Показу остаётся, ЧЕМ эти полосы рисовать.
##
## ПОЛОСА ШИРЕ ГОЛОВКИ ОБРЕЗАЕТСЯ ЕЮ, вместе с фасками. Так бывает у острогана
## остряка: в острие головки шесть миллиметров, а объявленная полоса — пятьдесят
## четыре. Обрезка здесь не подмена присланного: сервер и объявляет, что головка
## ýже наката бывает только у острогана и что полосу там режет само сечение.
static func head_bands(head_width_m: float, ride_m: float, fillet_m: float) -> Vector4:
	var band := clampf(ride_m, 0.0, head_width_m)
	var band_i := (head_width_m - band) * 0.5
	var fil := minf(fillet_m, band_i)
