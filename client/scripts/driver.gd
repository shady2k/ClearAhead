...
## ДОСЯГАЕМОСТЬ ПРИВОДА СТРЕЛКИ В ПЛАНЕ, метры.
##
## Меньше, чем до кабины (BOARD_REACH_M = 4.0), и это не вкус: у стрелки человек
## берётся рукой ЗА САМ МЕХАНИЗМ, а не тянется к посту на высоте полутора метров.
## Три метра — это шаг-другой от станины, то есть расстояние, с которого до неё
## и вправду дотягиваются.
##
## Сверху его ограничивает то же, что и досягаемость кабины: междупутье 5.3 м.
## Подсказка, дотянувшаяся до соседнего пути, предлагала бы перевести стрелку,
## стоя от неё через путь.
const STAND_REACH_M := 3.0

## Отступ от БОРТА машины, на который человек встаёт, выйдя из кабины, метры.
##
## Решение о теле, а не о мире: это ширина шага в сторону от подножки, того же
## класса, что рост и высота глаз. Полметра — человек стоит вплотную к машине и
## не внутри её габарита; больше — и он оказывался бы в междупутье.
const ALIGHT_CLEARANCE_M := 0.6
...
class Post extends RefCounted:
	...
	var index := 0
	## half_width — полуширина машины, метры. ИЗ ПАСПОРТА, а не из головы: чтобы
	## выйти из кабины наружу, надо знать, где у машины борт, и знает это тот же,
	...
	## door — ГДЕ ЧЕЛОВЕК ОКАЗЫВАЕТСЯ, ВЫЙДЯ ИЗ КАБИНЫ: у борта машины, со
	## стороны side (+1 — местный плюс X, −1 — минус), на уровне пола кабины.
	##
	## Дверей в модели нет, и это не мешает: дверь кабины — там, где кабина, то
	## есть на её продольной отметке (local.z). Отступ в сторону берётся от
	## ПАСПОРТНОЙ полуширины, а не от вида: борт у машины там, где объявлен
	## габарит.
	##
	## Точка ещё НЕ НА ЗЕМЛЕ: она на высоте пола, и опустить человека на твердь
	## обязан луч (_settle). Гадать про отметку земли под бортом нельзя — под ней
	## бывает балласт, платформа и междупутье.
	func door(side: float, clearance: float) -> Vector3:
		return node.to_global(local + Vector3(side * (half_width + clearance), 0.0, 0.0))
...
var _mode := MODE_EYE
## Вид, который был у игрока ДО посадки в кабину. Посадка забирает вид себе
## (board: внутри кузова смотреть не на что), и высадка обязана вернуть ровно
## тот, что забрала, а не назначить свой: игрок мог сидеть в обзоре нарочно.
## −1 значит «не забирали».
var _mode_before_board := -1
...
## Приводы стрелок, до которых можно дотянуться. Даёт мир — по той же границе,
## что и посты: где стоят стрелки, факт мира, а не свойство человека.
var _stands: Array = []
...
## С какой стороны машины человек садился, в осях ЕЁ узла: +1 — местный плюс X,
## −1 — минус. С той же стороны он и выйдет (alight).
var _boarded_side := 1.0
...
## set_stands — объявить человеку, до каких стрелок он вправе дотянуться.
##
## Тот же довод, что у set_posts: список даёт МИР. Человек отвечает на свой
## вопрос — дотянется ли, — и отвечает на него сам.
func set_stands(stands: Array) -> void:
	_stands = stands
	_refresh_prompt()


## nearest_stand — ближайший привод В ДОСЯГАЕМОСТИ, {stand, dist}. Пусто, если
## до ближайшего дальше STAND_REACH_M либо приводов нет вовсе.
##
## ДОСЯГАЕМОСТЬ ПРОВЕРЯЕТСЯ ЗДЕСЬ, а не у вызывающего, и это не мелочь: тем же
## ответом пользуются подсказка у ног и выбор цели пульта, и разойдись у них
## порог — панель предлагала бы перевести стрелку, до которой человек не достаёт.
##
## План, а не пространство: указатель поднят на полтора метра, и трёхмерное
## расстояние наказывало бы за высоту мачты, а не за то, что человек далеко.
func nearest_stand() -> Dictionary:
	var best: Variant = null
	var best_d := INF
	for st in _stands:
		var at: Vector3 = (st as SwitchStand).plan_point()
		var d := Vector2(at.x - global_position.x, at.z - global_position.z).length()
		if d < best_d:
			best_d = d
			best = st
	if best == null or best_d > STAND_REACH_M:
		return {}
	return {"stand": best, "dist": best_d}
...
func _refresh_prompt() -> void:
	...
	if _post != null:
		...
		# СТРЕЛКА — ВТОРЫМ ПРЕДЛОЖЕНИЕМ, а не вместо: у машины и у привода
		# досягаемости разные (4.0 против 3.0 м), и стоять в обеих сразу можно.
		# Обе строки показываются вместе — иначе, подойдя к стрелке у самого
		# локомотива, человек терял бы одно из двух действий без всякого объяснения.
		var stand := nearest_stand()
		if not stand.is_empty():
			var s_label: String = (stand["stand"] as SwitchStand).label
			var about := "стрелку %s" % s_label if s_label != "" else "стрелку"
			want = ("%s   ·   " % want if want != "" else "") + "T — перевести %s" % about
	if want == _prompt:
		return
	_prompt = want
...
func board(p: Post) -> void:
	# С КАКОЙ СТОРОНЫ ПОДОШЁЛ, С ТОЙ И ВЫЙДЕТ. Сторона запоминается в осях
	# МАШИНЫ, а не мира: машина поедет и повернётся, а борт, у которого человек
	# стоял, останется тем же бортом.
	var side := p.node.to_local(global_position).x
	_boarded_side = 1.0 if side >= 0.0 else -1.0
	_post = p
	velocity = Vector3.ZERO
	...
	_place_in_cab()
	if _mode != MODE_ORBIT:
		# ВИД ЗАПОМИНАЕТСЯ ПЕРЕД ТЕМ, КАК ЕГО ЗАБРАТЬ. Пара board/alight обязана
		# быть симметричной: всё, что посадка отобрала, высадка возвращает. До
		# 2026-08-15 возвращалась только клавиатура камеры, и вышедший из кабины
		# оставался в обзоре — «машинист после выхода продолжает управлять
		# камерой» (ClearAhead-xbia).
		_mode_before_board = _mode
...
## alight — выйти из кабины У БОРТА МАШИНЫ, а не туда, откуда садился.
##
## # Что здесь стояло и почему это была поломка
##
## Стоял возврат в _boarded_from — точку, где человек стоял в миг посадки.
## Довод был честный, пока машина не ездила: «место, откуда садились, —
## единственная точка, про которую ТОЧНО известно, что под ней твердь». Машина
## поехала, и довод умер вместе с ним: пройдя километр, машинист выходил на
## километр назад — то есть в поле, из которого машины уже не видно.
##
## Снаружи это выглядело КЛАВИШЕЙ, КОТОРАЯ НЕ РАБОТАЕТ, и именно так это и было
## названо: «машинист по E не может выйти из локомотива». Кадр при этом не
## менялся вовсе — обзорная камера оставалась наведённой на машину (её
## переставляет мир, world._look_at_driver), — и единственным признаком выхода
## была строка в углу.
##
## # Куда он выходит теперь
##
## К БОРТУ, у своей кабины, с той стороны, с которой садился (Post.door). Дверей
## в модели по-прежнему нет, и они не нужны: дверь кабины — там, где кабина, а
## где борт, говорит паспортный габарит.
##
## ЛУЧ ТЕПЕРЬ НУЖЕН: точка у борта взята на высоте пола кабины, и под ней бывает
## балласт, платформа и междупутье. Ставит на твердь _settle, тем же лучом, что и
## при появлении. put() здесь по-прежнему звать нельзя: он переписывает _home, то
## есть выход из кабины молча переносил бы точку возврата клавиши F.
func alight() -> void:
	if _post == null:
		return
	var was := _post
	_post = null
	velocity = Vector3.ZERO
	global_position = was.door(_boarded_side, ALIGHT_CLEARANCE_M)
	_settle_pending = true
	_pitch = START_PITCH_DEG
	_give_back_what_boarding_took()
	_apply_look()
	_refresh_prompt()
	_say("вышел из единицы %s" % was.unit_id)


## _give_back_what_boarding_took — вернуть то, что посадка забрала: клавиатуру
## камере и вид игроку.
##
## # Почему это отдельная функция, а не две строки в alight
##
## ВЫХОДОВ ИЗ КАБИНЫ ДВА, а возврат до 2026-08-15 стоял в одном. E звал alight и
## возвращал клавиатуру; F звал _go_home и не возвращал ничего — то есть
## вышедший телепортом оставался и в обзоре, и с мёртвыми буквами панорамы.
## Второй выход завели позже первого, и он унёс с собой ровно то, о чём легко
## забыть, когда возврат размазан по вызывающим.
##
## Вид возвращается ТОТ, ЧТО БЫЛ, а не MODE_EYE: игрок мог сидеть в обзоре
## нарочно, и высадка не вправе решать за него. Молча — по тому же доводу, что и
## в board: вид переключился сам, объявлять это игроку незачем.
func _give_back_what_boarding_took() -> void:
	if _camera != null:
		_camera.set_keys(true)
	if _mode_before_board >= 0:
		_apply_mode(_mode_before_board, false)
		_mode_before_board = -1
...
func _process(_delta: float) -> void:
	...
	if _mode == MODE_ORBIT:
		...
		#
		# ФОКУС ВЕДЁТСЯ И СНАРУЖИ, а не только у сидящего. Ходить в обзорном виде
		# нельзя (_physics_process выходит на MODE_ORBIT), и оттого казалось, что
		# вне кабины фокусу двигаться незачем. Двигается ЧЕЛОВЕК: выход из кабины
		# и телепорт F переставляют его, не спрашивая кадра, — и камера оставалась
		# смотреть на машину, из которой он только что вышел.
...
##
## Зовёт мир, когда человек вышел из кабины: пока он сидел, кадр был выставлен по
## машине (world._look_at_machine, полторы её длины — за полсотни метров), и
## оставить его так значило бы разглядывать вышедшего с другого конца станции.
##
## Азимут и наклон НЕ ТРОГАЮТСЯ: их выкрутил игрок, и они принадлежат ему —
## правило камеры, записанное там же, где она настраивается один раз (stand).
...
func _go_home() -> void:
	if _post != null:
		var was := _post
		_post = null
		# ТЕЛЕПОРТ ИЗ КАБИНЫ — ТОЖЕ ВЫХОД, и возвращает он то же самое, что
		# выход по E: клавиатуру камере и вид игроку (разбор — у
		# _give_back_what_boarding_took).
		_give_back_what_boarding_took()
