...
## платформу в один белый — измерено срезом UNSHADED.
const COL_BALLAST := Color(0.21, 0.21, 0.22)  # щебень 0.18-0.22, холодный серый
const COL_SLEEPER := Color(0.16, 0.11, 0.07)  # пропитанная древесина, тёплая и тёмная
const COL_RAIL := Color(0.26, 0.22, 0.19)     # ржавая сталь 0.15-0.20
const COL_FROG := Color(0.42, 0.30, 0.22)
const COL_GROUND := Color(0.24, 0.29, 0.16)   # трава/дёрн
const COL_EARTH := Color(0.33, 0.27, 0.19)    # земляные работы: грунт откосов
...
## Спайк отвечает на один вопрос: читается ли путь железной дорогой, когда под
## ним неровная земля, а сам он идёт по насыпи и в выемке.
const TERR_X0 := -260.0          # м — границы поля высот
const TERR_X1 := 660.0
const TERR_Z0 := -320.0
const TERR_Z1 := 320.0
...
var _cl_bb := Rect2()                 # габарит осевых в плане (x, z)

## Продольный профиль пути: подъём 8 промилле с переломом, всего ~2.6 м на
## станцию. Числа реалистичные, а не зрелищные: 8 промилле это меньше высоты
## локомотива на всю длину. Драму даёт не уклон, а насыпь над рельефом.
func _track_y(x: float) -> float:
	var g0 := 0.0
	if x <= 40.0:
		return 0.0
	if x <= 120.0:
		# вертикальная кривая: уклон линейно 0 -> 8 промилле
		var t := (x - 40.0) / 80.0
		var s := 0.008 * t
		return (g0 + s) * 0.5 * (x - 40.0)
	var z120 := 0.008 * 0.5 * 80.0
	if x <= 260.0:
		return z120 + 0.008 * (x - 120.0)
	var z260 := z120 + 0.008 * 140.0
	var t2 := clampf((x - 260.0) / 100.0, 0.0, 1.0)
	var s2 := 0.008 + (-0.004 - 0.008) * t2
	return z260 + (0.008 + s2) * 0.5 * (x - 260.0)

## Естественный рельеф до земляных работ: сумма синусов — гладко и
## детерминированно, без шумовых объектов.
func _ground_natural(x: float, z: float) -> float:
	return -4.2 \
		+ 3.2 * sin(x * 0.0165 - 0.4) \
		+ 2.4 * cos(z * 0.0230 + 1.3) \
		+ 1.5 * sin((x + z) * 0.0098 - 0.7) \
		+ 2.3 * cos(x * 0.052 + z * 0.031) \
		+ 1.3 * sin(x * 0.11 - z * 0.083) \
		+ 0.6 * cos(x * 0.19 + z * 0.16)

## Земля после земляных работ: у пути срезается до полки, вдали остаётся своей.
## СРЕЗАЕМ ТОЛЬКО ВНИЗ — насыпь это отдельное тело (_add_earthworks), а не
## поднятая земля. Это и есть «профиль минус рельеф» из спеки формата.
func _ground_worked(x: float, z: float) -> float:
	var nat := _ground_natural(x, z)
	if _cl_pts.is_empty():
		return nat
	...
	for p in _cl_pts:
		...
		if d2 < best:
			...
			best_y = p.y
	var d := sqrt(best)
	# ОБЩЕЕ ЗЕМЛЯНОЕ ПОЛОТНО: одна поверхность на всю площадку. Внутри полосы
	# — отметка бровки; дальше откос 1:1.5 до встречи с натурой, и вверх
	...
	if reach < 0.01:
		return shelf
	var t: float = clampf((d - CORR_INNER) / reach, 0.0, 1.0)
	t = t * t * (3.0 - 2.0 * t)
	return lerpf(shelf, nat, t)
...
func _mat(albedo: Color, roughness: float, metallic: float = 0.0, noisy: bool = false, tri: float = 0.05) -> StandardMaterial3D:
	...
	if noisy:
		...
		ramp.colors = PackedColorArray([Color(0.55, 0.55, 0.55), Color(1.35, 1.35, 1.35)])
		nt.color_ramp = ramp
		m.albedo_texture = nt
		# СПАЙК: меши из SurfaceTool идут БЕЗ UV (_quad не зовёт set_uv), поэтому
		# обычная текстура на них сэмплится в одну точку. Трипланар берёт
		# координаты из мировой позиции и работает без UV.
...
func _add_ballast(tool: SurfaceTool, el: Dictionary, lift: float) -> void:
	...
	_extrude(tool, _chain3d(el), section,
		not _element_connected_at_start(el), not _element_connected_at_end(el))
...
func _extrude(tool: SurfaceTool, chain: PackedVector3Array, section: PackedVector3Array, cap_start: bool = false, cap_end: bool = true) -> void:
	var n := section.size()
...
func _collect_centerlines() -> void:
	_cl_pts = PackedVector3Array()
	for el in _geometry.elements:
		var length := _element_length(el)
		var u := 0.0
		while u <= length + 1e-6:
			var pose := GM.pose_at(el.start, el.primitives, u)
			if pose.ok:
				_cl_pts.append(Vector3(pose.x, _track_y(pose.x), pose.y))
			u += CENTER_STEP
	if _cl_pts.is_empty():
		return
	_cl_bb = Rect2(_cl_pts[0].x, _cl_pts[0].z, 0.0, 0.0)
	for p in _cl_pts:
		_cl_bb = _cl_bb.expand(Vector2(p.x, p.z))

## Сетка вершин, натянутая мешем. Вместо плоскости — рельеф после земляных
## работ: вдали свой, у пути срезан до полки.
func _add_ground() -> void:
	var nx := int((TERR_X1 - TERR_X0) / TERR_STEP) + 1
	var nz := int((TERR_Z1 - TERR_Z0) / TERR_STEP) + 1
	var h := []
	h.resize(nx)
	for i in nx:
		...
		col.resize(nz)
		for j in nz:
			col[j] = _ground_worked(TERR_X0 + i * TERR_STEP, TERR_Z0 + j * TERR_STEP)
		h[i] = col
	...
	for i in nx - 1:
		...
		var x0 := TERR_X0 + i * TERR_STEP
		var x1 := x0 + TERR_STEP
		for j in nz - 1:
			var z0 := TERR_Z0 + j * TERR_STEP
			var z1 := z0 + TERR_STEP
			# Обмотка: проверено эмпирически — обратная отсекается CULL_BACK.
			_quad(tool,
				Vector3(x0, ci[j], z0), Vector3(x1, cj[j], z0),
				Vector3(x1, cj[j + 1], z1), Vector3(x0, ci[j + 1], z1))
	_commit(tool, _mat(COL_GROUND, 1.0, 0.0, true, 0.025))

## --- земляные работы: насыпь как отдельное тело ---
## Профиль минус рельеф. Где подошва балласта выше земли — откосы 1:1.5 от
## бровки вниз до земли. Где ниже — выемка, и её делает срез полки в _ground_worked.
func _add_earthworks(tool: SurfaceTool, el: Dictionary) -> void:
	var typ := _type()
	if typ.is_empty():
		return
	var hw := float(typ.ballast.half_width) + BALLAST_ADD
	var length := _element_length(el)
	var prev := {}
	var u := 0.0
	while u <= length + 1e-6:
		...
		if not pose.ok:
			break
		var d := Vector2(cos(pose.heading), sin(pose.heading))
		var lat := Vector3(-d.y, 0.0, d.x)
		var c := Vector3(pose.x, _track_y(pose.x) - BALLAST_DEPTH, pose.y)
		var cur := {}
		for k in 2:
			var sgn := 1.0 if k == 0 else -1.0
			var crest: Vector3 = c + lat * (hw * sgn)
			var gy := _ground_natural(crest.x, crest.z)
			var drop: float = maxf(crest.y - gy, 0.0)
			var toe: Vector3 = crest + lat * (drop * FILL_SLOPE * sgn)
			toe.y = gy
			cur[k] = [crest, toe, drop]
		if not prev.is_empty():
			for k in 2:
				var a: Array = prev[k]
				var b: Array = cur[k]
				if float(a[2]) < 0.05 and float(b[2]) < 0.05:
					continue
				var a0: Vector3 = a[0]
				var a1: Vector3 = a[1]
				var b0: Vector3 = b[0]
				var b1: Vector3 = b[1]
				if k == 0:
					_quad(tool, a0, a1, b1, b0)
				else:
					_quad(tool, a1, a0, b0, b1)
		prev = cur
		u += 2.0
	...
	for obj in _geometry.get("trackside", []):
		...
		for span in obj.spans:
			...
			_extrude(edge, chain, PackedVector3Array([
				...
				Vector3(ehi, yt, 0.0), Vector3(ehi, yb, 0.0)]), true, true)
...
func _add_buffer_stops(tool: SurfaceTool) -> void:
	...
	for el in _geometry.elements:
		...
		var c := Vector3(pose.x, _track_y(pose.x), pose.y) + dir * (BUFFER_W * 0.5)
...
func _element_connected_at_start(el: Dictionary) -> bool:
	var s0 := Vector2(float(el.start.plan.x), float(el.start.plan.y))
	for other in _geometry.elements:
		if other.id == el.id:
			continue
		var pose := GM.pose_at(other.start, other.primitives, _element_length(other))
		if not pose.ok:
			continue
		if Vector2(pose.x, pose.y).distance_to(s0) < 0.05:
			return true
	return false
...
func configure_camera(focus: Vector2, size: float, azimuth_deg: float, elev_deg: float) -> void:
	...
	_cam_ortho = not shot_persp
	_cam_size = size if size > 0.0 else 1.0
	if shot_persp:
		_cam_dist = size if size > 0.0 else 120.0
	_apply_camera()
	if size <= 0.0:
		_cam_size = _fit_size()
		_apply_camera()
