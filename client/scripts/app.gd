...
## кадре (разбор — в шапке driver.gd). С 2026-08-13 он вдобавок ВХОДИТ В КАБИНУ
## клавишей E — но и только: вести машину нечем, органов управления нет ни у
## клиента, ни в контракте. Прежняя редакция этой шапки говорила «поездов нет:
## подвижного состава не существует в модели мира»; первая половина верна
## по-прежнему, вторая устарела — состав приехал с сервера (ClearAhead-0na).
## Плоской схемы ДСП по-прежнему нет — она снесена вместе со старым клиентом и
## не воскрешена.
##
...
##   godot --path client -- … --role=driver --board                # сразу в кабине
extends Node
...
const ROLE_HINTS := {
	ShellUIScript.Role.DRIVER: "V — вид (обзор · от первого лица · от третьего) · E — сесть в кабину и выйти (подойдя к машине) · F — телепорт в точку появления · в видах от лица: мышь — взгляд, WASD — идти, Shift — бежать, пробел — прыжок · в обзоре: ЛКМ — орбита, WASD — панорама, колесо и двупальцевый скролл — зум · H — панель отладки · Esc — меню",
	ShellUIScript.Role.DSP: "ЛКМ — орбита · Shift+ЛКМ — панорама · WASD — панорама · колесо, щипок, двупальцевый скролл, +/− — зум · P — проекция · H — панель отладки · Esc — меню",
	ShellUIScript.Role.BUILDER: "ЛКМ — орбита · Shift+ЛКМ — панорама · WASD — панорама · колесо, щипок, двупальцевый скролл, +/− — зум · P — проекция · H — панель отладки · Esc — меню",
}
...
## Посадить машиниста в кабину сразу по готовности мира.
##
## Ключ существует ОТДЕЛЬНО от клавиши E и не дублирует её — та же пара, что
## --driver-view рядом с клавишей V: клавишей садится игрок, ключом — снимок и
## зонд, которым нажимать некому.
##
## И он НЕ СПРАШИВАЕТ ДОСЯГАЕМОСТИ, в отличие от клавиши. Человек появляется на
## середине платформы, а машина стоит в сотне метров от него: требовать от ключа
