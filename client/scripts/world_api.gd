...
##
## ПОЧИНЕНО 2026-08-15 (ClearAhead-s42): плитки едут РАЗОМ, по IN_FLIGHT штук в
## полёте. Правкой этой функции, как здесь и было записано, — рендер не тронут.
func terrain(region: String, addresses: Array) -> Array:
	return await _in_flight(addresses, func(a: Dictionary) -> Tile: return await tile(region, a))


## IN_FLIGHT — сколько запросов рельефа держим в полёте разом.
##
## # Число выбрано ЗАМЕРОМ, и вот замер (make net-probe, 2026-08-15, петля)
##
##	в полёте  1 (как было)   2070 мс на 60 запросов
##	в полёте  2              1214 мс   в 1.7 раза
##	в полёте  4               565 мс   в 3.7 раза
##	в полёте  8               275 мс   в 7.5 раза
##	в полёте 16               152 мс   в 13.6 раза
##	в полёте 32                83 мс   в 25.0 раза
##
## Рост почти линейный, и это не удача, а следствие причины: запрос из Godot
## стоит ПЯТЬ ОБОРОТОВ ГЛАВНОГО ЦИКЛА (HTTPRequest продвигает свой автомат по
## шагу за кадр), а не сетевого времени — сервер отдаёт тот же чанк за 218 мкс.
## Пока клиент упирается в кадры, N запросов проходят за те же пять оборотов.
##
## ВЗЯТО 16, А НЕ 32, и цена названа: тридцать два дали бы ещё вдвое, но это
## тридцать два одновременных соединения с сервером ради задачи, которая на
## шестнадцати уже перестала быть заметной (152 мс против 18 секунд, с которых
## всё начиналось). Число — одна константа, и поднять его можно новым замером, а
## не спором.
const IN_FLIGHT := 16

## _tile_ready — плитка доехала. ЧАСТНЫЙ сигнал: наружу он не значит ничего,
## внутри он единственный способ дождаться корутин, не опрашивая кадры.
signal _tile_ready


## _in_flight — прогнать адреса через make, держа IN_FLIGHT штук в полёте.
##
## ПОРЯДОК РЕЗУЛЬТАТА — ПОРЯДОК АДРЕСОВ, а не порядок ответов: вызывающий
## сопоставляет плитки со своим списком по месту, и «кто ответил первым» — это
## свойство сети, которому в его данных делать нечего.
##
## # Почему сигнал, а не опрос кадров
##
## Ожидание кадра стоило бы кадр на каждую доехавшую плитку — то есть вернуло бы
## часть той самой цены, ради которой всё делается. Сигнал будит ожидающего в тот
## же миг.
##
## Потерять пробуждение здесь нельзя, и это свойство однопоточности, а не
## везение: emit синхронно возобновляет ждущую корутину, та успевает запустить
## следующие запросы и снова встать на await ВНУТРИ того же вызова emit. Второй
## плитке физически некогда доехать между этими строками — сеть продвигается
## только на границах кадра, а мы кадра не отдаём.
func _in_flight(addresses: Array, make: Callable) -> Array:
	var out: Array = []
	...
	var sent := 0
	var got := 0
	while got < addresses.size():
		while sent < addresses.size() and sent - got < IN_FLIGHT:
			_fill(out, sent, addresses[sent] as Dictionary, make)
			sent += 1
		await _tile_ready
		got += 1
	return out
...
## _fill — одна плитка на своё место. Корутина: её ЗАПУСКАЮТ, не дожидаясь.
func _fill(out: Array, at: int, address: Dictionary, make: Callable) -> void:
	out[at] = await make.call(address)
	_tile_ready.emit()


## chunks — ВЫСОТЫ ЦЕЛЫМ СПИСКОМ АДРЕСОВ, разом и в порядке адресов.
##
## Тот же механизм, что у terrain, но без покрова и леса: их спрашивает тот, кто
## РИСУЕТ, а тому, кто меряет ПОКРЫТИЕ (проверка 90_coverage), нужны одни
## высоты. Заводить ради этого второй цикл в проверке значило бы завести второе
## место, где живёт порядок хождения в сеть, — то самое, от чего уводила
## ClearAhead-w6d.
func chunks(region: String, addresses: Array) -> Array:
	return await _in_flight(addresses, func(a: Dictionary) -> Heights:
		return await chunk(region, int(a["level"]), int(a["cx"]), int(a["cz"])))
...
func world_terrain(match: String, version: int, region: String, addresses: Array) -> Array:
	return await _in_flight(addresses,
		func(a: Dictionary) -> Tile: return await world_tile(match, version, region, a))
...
## Live — живое состояние партии ОДНИМ ЗАПРОСОМ.
##
## ИГРА ЭТИМ БОЛЬШЕ НЕ ПОЛЬЗУЕТСЯ с 2026-08-14: состояние приходит каналом
## (live_channel.gd), и мир берёт первое состояние из рукопожатия. Ручка и этот
## ответ остаются ради проверок, которые ходят по HTTP и сокета не держат
## (checks/live/25_rolling_stock.gd, 35_worlds.gd).
##
## Цена названа вслух: одно состояние отдаётся двумя способами. Уедет вместе с
## теми проверками, а не раньше — снести её сейчас значило бы ослепить их ради
## чистоты списка адресов.
