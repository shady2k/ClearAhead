...
func _run() -> void:
	...
	# 4б. ПОКРОВ. Хвост /cover у того же адреса чанка: клетка одна, и два пути к
	#     ней развели бы её тождество надвое.
	var cov: Dictionary = await net.fetch("/regions/%s/chunks/%d/%d/%d/cover" % [region, addr["level"], addr["cx"], addr["cz"]])
	_ok("покров 200", cov["ok"] and cov["code"] == 200, "код %s" % cov.get("code"))
	if cov["ok"] and cov["code"] == 200:
		var cb: PackedByteArray = cov["body"]
		...
		# База высот у покрова смысла не имеет: класс поверхности не
		# отсчитывается ни от чего. Лишний заголовок читался бы как «здесь есть
		# высоты» и врал.
		_ok("у покрова нет X-Chunk-Base-Z-Mm",
			NetClient.header_value(cov["headers"], "X-Chunk-Base-Z-Mm") == "")
		# Классы — из объявленного перечня. Неизвестный код означал бы, что
		# сервер и клиент разошлись версиями контракта, и это обязано быть видно
		# числом, а не странным цветом на экране.
		var bad_class := 0
		var lo := 99
		var hi := -1
		var seen := {}
	...
	# 4в. ЛЕС и ЭТАЛОН ФУНКЦИИ АДРЕСА.
	#
	#     Здесь впервые с 4b4d6a7 в проекте снова есть ЭТАЛОН — и он возможен
	#     ровно потому, что ForestJitter ЦЕЛОЧИСЛЕННАЯ. Прежний эталон контракта
	#     сверял вычисленные float64 байт в байт, не пережил смены машины
	#     (расхождение ~1e-13 при неизменном коде) и был снесён. Целочисленный
	#     хеш этому не подвержен: он совпадает точно, и сверять его прямым
	#     равенством законно.
	#
	#     Значения сняты с сервера (go test ./internal/chunk -run Jitter).
	#     Расхождение здесь означает, что два рендерера поставят деревья разной
	#     высоты в разные места одной ячейки.
	var golden := [
	...
	var fr: Dictionary = await net.fetch("/regions/%s/chunks/%d/%d/%d/forest" % [region, addr["level"], addr["cx"], addr["cz"]])
	_ok("лес 200 на уровне 0", fr["ok"] and fr["code"] == 200, "код %s" % fr.get("code"))
	if fr["ok"] and fr["code"] == 200:
		var fb: PackedByteArray = fr["body"]
		...
		if cov["ok"] and cov["code"] == 200:
			...
			var tr := Forest.trees(fb, cov["body"], PackedFloat32Array(), 0.0, rule.samples, addr["cx"], addr["cz"], rule.side_of(0))
	...
	var f1: Dictionary = await net.fetch("/regions/%s/chunks/1/0/0/forest" % region)
