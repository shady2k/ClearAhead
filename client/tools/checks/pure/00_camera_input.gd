...
func run() -> void:
	...
	# ПАРА «ПОСАДКА — ВЫСАДКА» СИММЕТРИЧНА, и проверяется именно это, а не вид
	# сам по себе. Посадка вид ЗАБИРАЕТ (внутри кузова смотреть не на что, отсюда
	# обзор), и до 2026-08-15 не возвращала: вышедший из кабины оставался в
	# обзоре, то есть «машинист после выхода продолжает управлять камерой»
	# (ClearAhead-xbia). Дефект найден игрой, а не проверкой, потому что вид
	# проверялся только клавишей V.
	var view_ashore := driver.mode_name()
	await _feed(_key(KEY_E))
	_ok("машинист: у машины E сажает в кабину", driver.is_boarded())
	_ok("машинист: в кабине вид обзорный",
		driver.mode_name() == Driver.MODE_NAMES[Driver.MODE_ORBIT],
	...
	_ok("машинист: выйдя, получает обратно тот вид, что был до посадки",
		driver.mode_name() == view_ashore,
		"было «%s», стало «%s»" % [view_ashore, driver.mode_name()])
	...
	# ВТОРОЙ ВЫХОД ВОЗВРАЩАЕТ ТО ЖЕ, ЧТО ПЕРВЫЙ. Выходов из кабины два, и до
	# 2026-08-15 возврат стоял в одном: телепортом человек выходил в обзор и с
	# отобранной клавиатурой панорамы.
	_ok("машинист: телепорт из кабины тоже возвращает вид",
		driver.mode_name() == view_ashore,
		"было «%s», стало «%s»" % [view_ashore, driver.mode_name()])
