...
func run() -> void:
	...
	#
	# ПРОВЕРЯЕТСЯ МАШИНА С КАБИНОЙ, а не первая в наборе. Пока в наборе был один
	# локомотив, разницы не было; с приходом полувагона stock[0] стал вагоном —
	# и проверка постов машиниста покраснела на машине, у которой кабины нет по
	# устройству. Кабину объявляет паспорт полем controls (у вагона его нет), и
	# спрашивать надо у него, а не у порядка сортировки.
	var t := (stock[0] as Dictionary)
	var cabbed := -1
	for i in stock.size():
		if (stock[i] as Dictionary).has("controls"):
			cabbed = i
			break
	_ok("в наборе есть машина с органами управления", cabbed >= 0,
		"паспортов %d, ни одного с controls" % stock.size())
	if cabbed >= 0:
		t = stock[cabbed] as Dictionary
	...
	_ok("у машины с органами управления есть хотя бы один пост", cabs.size() > 0,
		"постов %d" % cabs.size())
	...
	# ОБРАТНОЕ ТОЖЕ ОБЯЗАНО БЫТЬ ВЕРНО: у машины БЕЗ органов управления постов
	# машиниста нет. Иначе вид обещал бы кабину там, где её не существует, и
	# человек, севший в вагон, оказался бы внутри груза.
	for i in stock.size():
		var s_t := stock[i] as Dictionary
		if s_t.has("controls"):
			continue
		var s_a := by_name.get(String(s_t.get("appearance", "")), {}) as Dictionary
		_ok("у машины без органов управления нет постов машиниста",
			(s_a.get("cabs", []) as Array).is_empty(),
			"%s: постов %d" % [String(s_t.get("id", "")), (s_a.get("cabs", []) as Array).size()])
