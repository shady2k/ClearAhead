...
func run() -> void:
	...
	# ДВА АДРЕСА — ОДНО ТЕЛО. Здесь сторожится дефект u09k целиком.
	#
	# Ревизионный адрес отдаёт геометрию, скомпилированную из карты в памяти
	# (свежую всегда), версионный — замороженную публикацию из базы. Разойтись
	# им нечем: бутстрап сверяет лежащее тело с построенным и публикует новую
	# версию, как только они разошлись (worldgen.republishStaleNetwork). Пока
	# этой сверки не было, расхождение было НЕВИДИМО: сервер отдавал старое
	# тело как исправное, снимок снимался со свежего адреса, проверки зеленели,
	# а игрок смотрел на мир трёхкоммитной давности.
	#
	# Сравниваются ТЕКСТЫ, а не словари: оба тела печатает один сериализатор
	# сервера, порядок ключей у них общий, и разошедшееся число видно глазом в
	# отчёте — тогда как Dictionary.hash() сказал бы только «не равны».
	var by_rev: WorldApi.Network = await ctx.api.network(ctx.region, await ctx.revision())
	_ok("сеть по ревизии получена", by_rev.have(), by_rev.reason)
	if net_res.have() and by_rev.have():
		var frozen := JSON.stringify(net_res.data, "", false, true)
		var live := JSON.stringify(by_rev.data, "", false, true)
		# Подробность НЕЙТРАЛЬНА: отчёт печатает её и у прошедшей проверки, и
		# приговор в ней читался бы как приговор исправному серверу.
		_ok("опубликованное тело сети совпадает со свежескомпилированным",
			frozen == live,
			"версия %d: %d байт, по ревизии %d" % [v, frozen.length(), live.length()])
