...
func _check_turnout(ch: LiveChannel, doc, snaps: Array) -> void:
	...
	if set_ok.is_empty():
		return
	# ОТВЕТ НАЗЫВАЕТ ЦЕЛЬ, А НЕ ВСТАВШЕЕ ПОЛОЖЕНИЕ. С 2026-08-16 перевод идёт
	# время, и стоящего положения сразу после команды не существует: стрелка в
	# переводе не стоит нигде. Правило ответа при этом прежнее — он говорит о
	# состоянии мира, а не повторяет просьбу.
	_ok("ответ несёт ветвь, к которой идёт остряк",
		String((set_ok[0] as Dictionary)["p"]) == want, str(set_ok[0]))

	var came := await _until(func() -> bool: return snaps.size() > before)
	_ok("снапшот принёс перевод сам", came)
	if came:
		var fresh: LiveChannel.Snapshot = snaps[snaps.size() - 1]
		var seen := {}
		for raw in fresh.turnouts:
			...
			if String(t.get("id", "")) == sw_id:
				seen = t
		# СНАПШОТ ЛОВИТСЯ ПОСРЕДИ ХОДА, и потому проверяется не положение, а
		# перевод: у ручного механизма остряк идёт секунды, и снапшот, пришедший
		# сразу после команды, застаёт его на пути.
		_ok("в снапшоте та же стрелка переводится к запрошенной ветви",
			bool(seen.get("moving", false)) and String(seen.get("to", "")) == want,
			"было %s, снапшот %s" % [was, str(seen)])
		_ok("идущая стрелка не стоит нигде", String(seen.get("position", "")) == "",
			str(seen.get("position", "")))
		# ДОЛЯ ПЕРЕВОДА ПРИЕЗЖАЕТ ЧИСЛОМ и лежит в своих границах: по ней клиент
		# откладывает остряк, и доля вне [0, 1] выломала бы его за ход.
		var pr := float(seen.get("progress", -1.0))
		_ok("доля перевода в границах", pr >= 0.0 and pr <= 1.0, "%.3f" % pr)

		# ЧТО ОСТРЯК ДОХОДИТ ДО КОНЦА, ЗДЕСЬ НЕ ПРОВЕРЯЕТСЯ, и это не пропуск.
		# Срок ожидания суиты (WAIT_S) отмерен под оборот по сети, а перевод
		# ручного механизма идёт секундами модельного времени; растянуть общий
		# срок ради одного ожидания значило бы замедлить каждую проверку рядом.
		#
		# Доведение до конца проверяют двое: тест сима (шаг за шагом, без сети) и
		# живой зонд стрелки (make turnout-probe, вся цепочка от клавиши).
