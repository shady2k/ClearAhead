...
//
// # Почему это не зовёт Load, хотя предел взят из ГОСТ
//
// Потому что первым его нарушает НАШ ЖЕ боевой локомотив: у ВЛ80 в паспорте
// 3.63 × 5.40 м, и это не машина, а ЗАМЕР ЧУЖОГО МЕША, умноженный на масштаб
// подгонки под колею (бида ClearAhead-w4q, заведена 2026-08-12). Там же
// записано предсказание, которое сегодня и сбылось: «когда появятся проверки
// габарита, она их не пройдёт».
//
// Выбор из двух: подставить локомотиву правдоподобные числа — то есть сделать
// ровно то, за что эта проверка и заведена, — либо оставить её невключённой,
// пока паспорт ВЛ80 не получит числа ИЗ ИСТОЧНИКА. Выбрано второе: проверка,
// ради зелени которой подделали данные, хуже отсутствующей.
//
// ВКЛЮЧАЕТСЯ ОДНОЙ СТРОКОЙ в Load, как только w4q закрыта. До тех пор её зовут
// тесты — на фикстуре, где числа наши и заведомо верные.
