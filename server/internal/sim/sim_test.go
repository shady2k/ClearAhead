...
// между плоскостями автосцепок, значит половина машины (у ВЛ80 16.42 м)
...
// расстановка кладёт тело в ОДИН элемент (startSpan), а ВЛ80 длиной 32.84 м в
// проход 33.5 м помещается впритык лишь серединой на середине прохода — то
// есть практически не ставится. Это не обход проверки, а её точное условие: на
...
func TestMachineOnTheTurnoutDoesNotSeeItAhead(t *testing.T) {
	...
	// полдлины позади него: u < 16.42 значит, что конец уже за границей элемента.
