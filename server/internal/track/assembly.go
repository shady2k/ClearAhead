...
// ЧТО ДЕТАЛЬ ДЕЛАЕТ ДЛЯ КОЛЕСА. Не «из чего она», а «зачем она под колесом», и
// это разные вопросы: контррельс — такой же рельс того же профиля, но колесо по
// нему не катится никогда.
//
// # Зачем признак заведён
//
// Без него проверка непрерывности спасалась ПОСТОРОННИМ МЕТАЛЛОМ: любая деталь,
// попавшая в окно, закрывала обрыв — контррельс, накладка, что угодно. Проверка
// отвечала «металл есть» на вопрос «есть ли путь под колесом», а это разные
// вопросы, и второй строже.
//
// # Почему три, а не два
//
// Разделение подсказано разбором с codex 2026-08-16 и держится на крестовине: там
// ОПОРА И ВЕДЕНИЕ РАЗДЕЛЯЮТСЯ ФИЗИЧЕСКИ. Усовик с сердечником несут колесо,
// контррельс в это же время ведёт колёсную пару за гребень противоположного
// колеса. Свести их в одно значило бы объявить, что контррельс держит вес, —
// и однажды на этом построить вывод о том, где поезд может ехать.
const (
	// RoleCarry — по головке этой детали КАТИТСЯ колесо.
	RoleCarry = "carry"
	// RoleGuide — деталь ВЕДЁТ колёсную пару гребнем и веса не несёт.
	RoleGuide = "guide"
	// RoleHold — деталь ДЕРЖИТ другие детали и ходовой функции не имеет вовсе:
	// накладки, вкладыш корневого крепления, подкладки.
	RoleHold = "hold"
)

// Виды обязательств. Пока проверяется одно, но названы все: перечень —
// это карта того, что доказано и что нет, и молчание о недоказанном есть
// ровно та болезнь, ради которой заведён этот файл.
//
// Словарь и разделение — разбор с codex 2026-08-16.
const (
	// ObligationRunningSurface — ходовая поверхность продолжается: головка,
	// по которой катится колесо, не обрывается и не прыгает вбок.
	ObligationRunningSurface = "running_surface_continuity"
	// ObligationWheelSupport — колесо ОПЁРТО: у крестовины ходовая поверхность
	// законно передаётся с усовика на сердечник, и опора при этом держится обеими
	// деталями разом. Проверяется существованием опоры, а не продолжением одной
	// детали. НЕ ДОКАЗЫВАЕТСЯ ПОКА.
	ObligationWheelSupport = "wheel_support_continuity"
	// ObligationWheelGuidance — колёсная пара ВЕДЁТСЯ: гребень не попадает в чужой
	// жёлоб. Дело контррельса. НЕ ДОКАЗЫВАЕТСЯ ПОКА.
	ObligationWheelGuidance = "wheel_guidance_continuity"
	// ObligationStructural — тела скреплены. Отношение ДЕТАЛЕЙ, а не свойство
	// станции: «рядом есть металл» силовой связи не доказывает. НЕ ДОКАЗЫВАЕТСЯ.
	ObligationStructural = "structural_connection"
	// ObligationFlangeClearance — жёлоб объявленной ширины существует и профиль
	// гребня в него проходит. Требование ПОЛОЖИТЕЛЬНОЕ, а не освобождение от
	// проверки: старое имя permitted_flangeway читалось как индульгенция.
	// НЕ ДОКАЗЫВАЕТСЯ ПОКА.
	ObligationFlangeClearance = "flange_clearance"
)

// PartRole называет, что деталь делает для колеса.
//
// Выводится из вида, а не хранится полем: вид детали и есть её назначение, и
// второе место, где это записано, разошлось бы с первым.
func PartRole(kind string) string {
	switch kind {
	case PartCheckRail:
		return RoleGuide
	default:
...
// Proven называет обязательства, которые проверка ДЕЙСТВИТЕЛЬНО доказала.
//
// Метод, а не комментарий, и это существенно: потребитель проверенной сборки
// иначе вправе решить, что доказано всё, чему есть имя. Молчание о недоказанном
// — та самая болезнь, ради которой заведён файл (BladeRootJoint существовал
// словом и не существовал телом), и повторять её на уровень выше нельзя.
//
// Список короткий намеренно. Остальные обязательства названы константами и не
// доказываются ничем: у опоры своя арифметика (у крестовины ходовая поверхность
// законно передаётся, а опора держится обеими деталями разом), у ведения —
// профиль гребня, у силовой связи — отношение деталей, а не станция.
func (v *ValidatedAssembly) Proven() []string {
	return []string{ObligationRunningSurface}
}
...
func metalUnionAt(parts []Part, element string, u, face, window float64) []span {
	...
	for _, p := range parts {
		...
		// профилем, и до этого различия он закрывал чужие обрывы: проверка
		// отвечала «металл есть» на вопрос «есть ли путь под колесом».
...
func findBreaks(parts []Part, ports []Port, gaps []Gap) []Break {
	...
	for i, p := range ports {
		...
		// У НЕНЕСУЩЕЙ ДЕТАЛИ вопрос о ходовой поверхности не имеет смысла:
		// контррельс её и не продолжает, он ведёт. Своё обязательство
		// (ObligationWheelGuidance) пока не доказывается ничем, и это записано
		// перечнем видов обязательств, а не молчанием здесь.
		if PartRole(p.Kind) != RoleCarry {
			continue
		}
		best := -1
