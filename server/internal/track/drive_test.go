...
// TestDriveSideFollowsGeometry — сторона выводится ИЗ ГЕОМЕТРИИ: боковой путь
// уходит влево, значит привод обязан уйти вправо.
//
// Половина проверки, которая раньше подавала сюда ЛЖИВУЮ пометку («right» при
// дуге влево), уехала в TestHandContradictingGeometryIsRefused: такая карта
// теперь не компилируется вовсе, и выводить по ней сторону не у кого.
func TestDriveSideFollowsGeometry(t *testing.T) {
	els := frogEls(mustChain(t, primStraight(t, 33.5)), mustChain(t, primArc(t, 300, +0.1107)))
	left := sw1Right()
	left.Hand = mapfmt.HandLeft
	d, err := turnoutDrive(els, timberTypes(5.5), frogConstruction(), left)
	if err != nil {
		t.Fatalf("привод стрелки: %v", err)
	}
	if d.Offset >= 0 {
...
// TestHandContradictingGeometryIsRefused — карта, где пометка о рукости
// разошлась с тем, куда боковой путь уходит НА САМОМ ДЕЛЕ, не компилируется.
//
// # Почему отказ, а не «геометрия сильнее»
//
// Потому что источников у стороны схода два, и оба работают: крестовина строит
// пересечение ниток ПО ПОМЕТКЕ (frog.go), брусья и привод — ПО ГЕОМЕТРИИ.
// Разойдись они, станция вышла бы наполовину правой, наполовину левой, и никакая
// из двух половин не была бы виновата. Прежний комментарий здесь утверждал, что
// такая карта «невозможна у валидатора», — она была возможна, и это ровно тот
// случай, когда правило записано, но не поставлено сторожем.
//
// Сверх того сторона схода уезжает КЛИЕНТУ (match.TurnoutState.Hand): по ней
// рисуется стрела указателя и развилка на пульте. Отдавать наружу пометку,
// которую никто не сверял с рельсами, значило бы показать машинисту стрелку,
// смотрящую не туда.
func TestHandContradictingGeometryIsRefused(t *testing.T) {
	els := frogEls(mustChain(t, primStraight(t, 33.5)), mustChain(t, primArc(t, 300, +0.1107)))
	lying := sw1Right() // пометка говорит "right", дуга уходит влево
	if _, err := turnoutDrive(els, timberTypes(5.5), frogConstruction(), lying); err == nil {
