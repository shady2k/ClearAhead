...
//
// # И ровно поэтому за изломом нужен присмотр (правка 2026-08-17)
//
// Объявленный излом допуском не проверяется — в этом весь смысл объявления. Цена
// такова: неверно ПОСТАВЛЕННЫЙ излом неотличим от верного, и первая же редакция
// поставила его во всех портах стрелки сразу, включая пятку. Проверка замыкания
// молчала сутки, а боковой путь уходил маркой 1/11.50 вместо 1/9.
//
// Вывод, который стоит держать: у объявленного свойства обязан быть тест на
// МЕСТО объявления, а не только на величину (turnout_test.go).
...
//	стрелка, общий порт:      straight --same+β0--> diverging
//	                          внешнее ребро --flip(+β0 боковому)--> обоим проходам
//	стрелка, порт S:          внешнее ребро --flip--> straight
//	стрелка, порт D:          внешнее ребро --flip--> diverging
//
// β0 СТОИТ РОВНО В ОДНОЙ СТРОКЕ ЭТОЙ ТАБЛИЦЫ — в общем порту. В пятке его нет:
// там проход уже повернул на всю марку, и ребро продолжает его касательно.
func buildJunctions(m *mapfmt.Map, els map[string]Element) (Junctions, error) {
	...
	for _, port := range ports {
		...
		for _, p := range passages {
			// ИЗЛОМ ЖИВЁТ ТОЛЬКО В ОСТРИЕ. До 2026-08-17 β0 прикладывался в
			// КАЖДОМ порту стрелки, и на пятке бокового прохода это было чистой
			// ошибкой: там проход уже повернул на всю марку, и внешнее ребро
			// обязано продолжить его касательно.
			//
			// Замер по ST_A: боковой проход SW1 кончается курсом −0.110658
			// (= arctan 1/9, как и положено), а ребро E_CROSS начиналось с
			// −0.086737. Разница — те самые β0 = 1°22'14", и за пятой боковой
			// путь уходил от прямого под маркой 1/11.50 вместо объявленной 1/9.
			// На двадцатиметровом ребре это 0.478 м в сторону, и всё, что за ним
			// (SW2, съезд, тупик), уезжало следом.
			//
			// Почему ошибку не поймал допуск замыкания: излом объявлен ЗАКОННЫМ
			// свойством порта (PortRelation.HeadingDelta), то есть не невязкой.
			// Объявленную ложь допуск не ловит по определению — её ловит только
			// вопрос «а в этом ли порту она законна».
			delta := 0.0
			if tp.role == "common" {
				delta = divergingDelta(p, tp.turnout, initialAngle)
			}
			link(external[0], p, true, delta)
		}
	}
	return j, nil
}
