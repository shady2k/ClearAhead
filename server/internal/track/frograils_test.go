...
func TestCastingWidensAsTheMarkPrescribes(t *testing.T) {
	...
	for _, f := range rg.Features {
		...
		for _, s := range []float64{0.5, 1.5} {
			...
			if got := w / s; math.Abs(got-want) > 0.02 {
				t.Fatalf("сердечник на %.1f м за остриём шириной %.4f м (%.4f на метр), а марка %s требует %.4f",
					s, w, got, m.Topology.Turnouts[0].Frog, want)
			}
		}
