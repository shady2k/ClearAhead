...
func (a *chunksAPI) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// (worldstore.PutChunk), поэтому не может описать чужие байты. Статика
	// меняется только со сменой ревизии чанка, а ревизия входит в адрес не
	// URL'ом, а хешем, — но кэш всё равно immutable: содержимое под этим
	// хешем неизменно, а ревалидацию обеспечивает If-None-Match.
	etag := `"` + c.Hash + `"`
