...
func construction(runs []mapfmt.ConstructionRun) *mapfmt.Construction {
	return &mapfmt.Construction{
		...
		Types: []mapfmt.TrackType{{
			...
			// ОСТРЯК. Оба числа с названным происхождением, и оба
			// ПРЕДВАРИТЕЛЬНЫЕ в том же смысле, что и брусья.
			//
			//   8.30 м — длина остряка обыкновенного перевода Р65 марки 1/9
			//     (проект 2750). Здесь она свойство ТИПА ПУТИ, а не марки, — то
			//     же приближение, что у timber.length_max, и та же цена: на
			//     карте с двумя марками обе получат один остряк.
			//   0.152 м — ход остряка по ПТЭ: норма на всю сеть колеи 1520.
			//     Число карты, а не константа кода, потому что у узкой колеи
			//     оно другое.
			Switch: &mapfmt.TrackSwitch{BladeLength: 8.30, Throw: 0.152},
			// КРЕСТОВИНА. Желоба — НОРМЫ ПТЭ и названы нормами: 46 мм в
			// крестовине, 44 мм у контррельса. Остальные четыре числа —
			// ОЦЕНКИ порядка комплекта марки 1/9 на Р65, и норм за ними нет:
			// длина усовика около двух метров, контррельса — четырёх с
			// половиной, отгиб четверть метра, раструб 86 мм.
			//
			// Разница в статусе важнее значений: желоба поменяются только
			// вместе с нормой, а оценки — вместе с первым же справочником
			// комплекта, который у нас появится.
			Frog: &mapfmt.TrackFrog{
				Flangeway:      0.046,
