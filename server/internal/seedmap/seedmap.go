...
func WithoutConstruction() Option {
	// Блок убирается ЦЕЛИКОМ, и с 2026-08-17 это снова просто.
	//
	// Пока каталог проектов лежал в карте рядом с решёткой, здесь была оговорка:
	// типы пути и прогоны убрать можно, а проект перевода нет — карта без
	// решётки со стрелкой без проекта была бы картой, у которой устройство есть,
	// а какое, неизвестно. Проект уехал в каталог сервера, стрелка ссылается на
	// него именем, и вопрос «какая здесь марка» решёткой больше не держится.
	return func(m *mapfmt.Map) { m.Construction = nil }
}
...
// TurnoutProjectID — проект перевода, на который ссылаются обе стрелки затравки.
//
// Строка каталога СЕРВЕРА (mapfmt/turnout_catalog.go), а не UUID записи в самой
// карте, как было до 2026-08-17: эпюра перестала быть частью карты, и
// генерировать для неё идентификатор больше некому и незачем.
const TurnoutProjectID = "r65-1-9-2434"

// TurnoutProjectForTest отдаёт проект перевода затравки БЕЗ всей карты.
//
...
// чисел в тестах при этом не появляется — это ровно та же запись каталога.
//
// Паника, а не ошибка: ссылка ведёт в каталог СЕРВЕРА, и её неразрешимость есть
// поломка сборки, а не состояние данных.
func TurnoutProjectForTest() mapfmt.TurnoutType {
	t, err := mapfmt.TurnoutProjectByID(TurnoutProjectID)
	if err != nil {
		panic(err)
	}
	return t
...
func turnout(id, name, drive string) mapfmt.Turnout {
	return mapfmt.Turnout{
		...
		Hand: "right",
		Frog: "1/9",
		// Ссылка на ПРОЕКТ перевода в каталоге сервера. Умолчания у неё нет:
		// марка — решение автора станции, и подставить её значило бы выбрать за
		// него проект.
		TurnoutType: TurnoutProjectID,
		Drive:       drive,
