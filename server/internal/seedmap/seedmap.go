...
// turnoutGeometry — геометрия обыкновенного перевода Р65 1/9 вправо ПО ЭПЮРЕ
// ПРОЕКТА 2434.
//
// # Что здесь стояло раньше и почему это было неправдой
//
...
// кончается прямой вставкой перед крестовиной.
//
// # Эпюра, и она сходится сама с собой
//
//	начальный угол остряка β0   1°22'14" = 0.023921 рад — в каталоге типов
...
//
// Проверка, которую никто не закладывал: β0 + 6.515/300 + 13.008/200.06 =
// 0.110658, а arctan(1/9) = 0.110657. Сходится. Ту же сумму теперь требует
// валидатор (checkTurnoutTurnAgreement), поэтому разъехаться молча они больше не
// могут.
//
// # Откуда длина прохода
//
// 27.504 м — от ОСТРИЯ остряка до торца крестовины. Выведено из эпюры, а не
// взято оттуда: она даёт a = 11.832 м от переднего стыка рамных рельсов до ЦП и
// b = 19.203 м от ЦП до торца, а острие отстоит от ЦП на 8.301 м (пересечение
// осей). Отсюда остриё в 3.531 м от переднего стыка и торец в 27.504 м от
// острия. Расхождение осей у математической точки крестовины при этом выходит
// 1.5200 м — ровно колея, и это вторая сходимость, которую никто не закладывал.
//
// # Чего в этой геометрии по-прежнему НЕТ
//
// Начальный угол здесь не записан: он свойство ПОРТА, а не траектории, и живёт в
// каталоге типов устройств. Проход начинается уже отклонённым — излом ставит
// распространение поз (track/propagate.go).
func turnoutGeometry() mapfmt.TurnoutGeometry {
	return mapfmt.TurnoutGeometry{
		Straight: mapfmt.Alignments{Horizontal: []mapfmt.HPrim{{Kind: "straight", Length: 27.504}}},
		Diverging: mapfmt.Alignments{Horizontal: []mapfmt.HPrim{
			{Kind: "arc", Radius: 300.00, Angle: -0.0217167},
...
func turnoutType() mapfmt.TurnoutType {
	return mapfmt.TurnoutType{
		...
		Switch: mapfmt.TurnoutSwitch{
			...
			Throw:                0.152,
			// β0 эпюры: 1°22'14". Модуль — знак ставит рукость.
			InitialAngle: 0.0239211,
		},
		// ОР65 — ОСТРЯКОВЫЙ РЕЛЬС, и он не Р65.
		//
		// Размеры по ГОСТ 17507-85 / ГОСТ Р 55820-2013: высота 140 (у Р65 180),
		// подошва 132 (150), головка 77.33 (75), шейка 58 (18), короткое плечо
		// подошвы 18. Профиль АСИММЕТРИЧЕН: головка вынесена относительно оси
		// подошвы в сторону рамного рельса, и подошва с той стороны усечена.
		//
		...
		// внутренней рабочей грани наружу, y от поверхности катания вниз.
		// Подошва остряка приходится на y = −0.140, то есть ВЫШЕ подошвы Р65 на
		// −0.180: остряк лежит на подушке, и в плане их подошвы не мешают друг
		// другу именно поэтому.
		BladeRail: mapfmt.TrackRail{
			Height:    0.140,
			HeadWidth: 0.07733,
