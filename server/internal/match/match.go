...
type Unit struct {
	...
	// LoadT — СКОЛЬКО ГРУЗА В ЭТОЙ МАШИНЕ, тонны.
	//
	// # Почему в расстановке, а не в паспорте и не в состоянии партии
	//
	// Паспорт отвечает «сколько машина ПРИНИМАЕТ» — это свойство класса
	// (content.FreightSpec.PayloadT). Сколько в ней лежит СЕЙЧАС — свойство
	// экземпляра, и в паспорте ему места нет: два одинаковых полувагона на
	// соседних путях бывают гружёный и порожний.
	//
	// В документе, а не в состоянии, потому что это ТО ЖЕ, что место и тип:
	// автор ставит на путь гружёный вагон, и мир начинается с него. Погрузка и
	// выгрузка сделают груз ещё и состоянием — тогда, когда появятся; до тех
	// пор состояние, заведённое «на будущее», было бы нулём, неотличимым от
	// незаполненного.
	//
	// ОБЯЗАТЕЛЕН У МАШИНЫ С ГРУЗОМ И ЗАПРЕЩЁН У ПРОЧИХ. Ноль пишется ЯВНО и
	// означает порожнюю — отсюда указатель, а не float64: разница между
	// порожним и гружёным вагоном это 70 тонн, тормозной путь и то, сдвинет ли
	// его локомотив вообще, и забытое поле обязано быть отличимо от «пусто».
...
type Match struct {
	...
	Region string
	Units  []Unit
	// Consists — СЦЕПЫ партии: то, что движется. Каждая единица входит ровно в
	// один, одиночная машина — сцеп из одной. Разбор — в consist.go.
	//
	// Порядком, а не картой: порядок сцепов устойчив между снимками, как и
	// порядок единиц, и человек, читающий два снимка подряд, читает их глазами.
	Consists []Consist `json:"-"`
...
func (m *Match) place(list []Unit, net *track.CompiledNetwork, set *content.Set) error {
	...
	for i, u := range list {
		...
		// ГРУЗ СВЕРЯЕТСЯ С ПАСПОРТОМ ЗДЕСЬ, до всякой геометрии: расстановка,
		// назвавшая груз машине, которая его не возит, — испорченный документ, и
		// читать её дальше незачем. AtLoad и отказывает: он один знает
		// грузоподъёмность, и второй проверки «сколько влезет» не заводится.
		switch payload, freighted := st.Freighted(); {
		case freighted && u.LoadT == nil:
			return fmt.Errorf("match: единица %s: тип %s возит груз, а поле load не названо; "+
				"порожняя пишется нулём, потому что забытое поле от порожней вагонетки неотличимо",
				mapfmt.Labeled(u.Name, u.ID), u.Type)
		case !freighted && u.LoadT != nil:
			return fmt.Errorf("match: единица %s: у типа %s грузоподъёмности нет, а расстановка назвала груз %.1f т",
				mapfmt.Labeled(u.Name, u.ID), u.Type, *u.LoadT)
		case freighted:
			if _, err := st.AtLoad(*u.LoadT); err != nil {
				return fmt.Errorf("match: единица %s: %w (грузоподъёмность %.1f т)",
					mapfmt.Labeled(u.Name, u.ID), err, payload)
			}
		}
		...
		// СЦЕП ЗАВОДИТСЯ ВМЕСТЕ С МАШИНОЙ, потому что движется именно он.
		// Единица вне сцепа — состояние, которого в этом мире не бывает: сцепка
		// меняет СВЯЗЬ между машинами, а не способ их считать (consist.go).
		m.SetConsist(Single(u.ID))
	}
