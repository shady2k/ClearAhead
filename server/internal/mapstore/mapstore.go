...
// разрешение симлинков, проверка путей. Каталога нет и не вернулось: карта
// приходит ОДНА и с ключа запуска (`clearahead -map`), а не выбирается из
// списка. Мир живёт в базе (worldstore); заготовка новой карты по-прежнему
// строится кодом (New ниже), потому что у пустой карты нет автора.
//
// Сам файл карты вернулся 2026-08-12 — разбор того, что при этом отменено, а что
// осталось в силе, лежит в шапке mapfmt/decode.go.
//
...
//
// Клиент больше не может прислать серверу карту документом, и возврат файла
// этого не изменил: ручек приёма карты нет. Карта входит в мир только с диска
// сервера, то есть решением того, кто сервер запускает. Клиенту она по-прежнему
// УЕЗЖАЕТ — направление осталось одно.
package mapstore
